...
Resolution order per key:
    ...
    2. 1Password Vault vault via `op read` / `op inject` (service account token)
...
def _op_read(key: str, vault: str = "Vault") -> str | None:
...
def get_secret(key: str, *, vault: str = "Vault") -> str | None:
...
def hydrate_env(keys: list[str] | None = None, *, vault: str = "Vault") -> dict[str, bool]:
