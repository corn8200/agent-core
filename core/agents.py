...
dispatch = AgentDefinition(
    description="Communications & sending specialist — email drafting, follow-ups, contact coordination via mail-queue (business) and SMTP (personal). Uses Bash + Read/Write to compose and ship messages, plus Mail.app via osascript for inbox checks. Pick when the deliverable is a sent message, not research. No web search, no Edit — composes from given facts. 10-turn budget, sonnet.",
...
herald = AgentDefinition(
    ...
    description="Brand & deliverable QC — reads brand-kit tokens, enforces taste + industry-standard quality rules on resumes, proposals, decks, client reports, websites, emails, and any other artifact. Runs mechanical QC (page count, fill ratio, section-across-page splits, ATS text extraction via pdftotext, brand-token drift) and auto-fixes mechanical violations by editing content (compress/expand bullets, restructure sections, adjust page breaks). Blocks + reports on subjective violations: images in ATS resumes, wrong tier tokens, fancy fonts where plain is required, brand voice drift, factual errors in identifiers. Uses Read/Write/Edit/Bash/Grep/Glob. Pick for any pre-delivery review or brand-consistency check. 25-turn budget, opus, max effort.",
