...
@tool(
    "send_business_email",
    "Send a business email from info@sentryaithermal.com via mail-queue on VPS.",
...
async def send_business_email(args: dict[str, Any]) -> dict:
    try:
        ...
        queue_cmd = (
            f"cd /srv/apps_REDACTED/mail-queue && "
...
@tool(
    "send_personal_email",
    "Send a personal or system email from  (). Use for briefs, alerts, family communications.",
...
async def send_personal_email(args: dict[str, Any]) -> dict:
    ...
    if args.get("_require_approval") and not args.get("_approved"):
        ...
        if not _is_self(to):
            ...
            if status == "sent_direct":
                return result.get("result") or {"content": [{"type": "text", "text": "sent"}]}
    helper = Path.home() / "claude-config" / "skills" / "communication" / "scripts" / "send-email.py"
    if helper.exists():
        cmd = [
            str(helper),
            "--from", "notify",
