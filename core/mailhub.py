...
This mirrors the canonical VPS helper at ``/path/to/lib/mailhub.py`` so
...
_TOKEN_ENV_FILES: tuple[Path, ...] = (
    ...
    Path("/etc/cp-api.env"),
    Path("/path/to/mailhub/config/mailhub.env"),
)
_OP_REF = "op://Vault/MAILHUB_TOKEN/password"
_OP_TIMEOUT_S = 5.0
    ...
    def __init__(self, sources_tried: list[str]):
        ...
        super().__init__(
            ...
            (
                ...
                "/path/to/mailhub/config/mailhub.env, or store as "
                f"{_OP_REF}."
            ),
            body=None,
        )
