...
`/path/to/taskqueue/tasks/vector.py` — keep behaviour in sync if that changes.
...
def _pg():
    dsn = os.environ.get("VEC_PG_DSN") or get_secret("VEC_PG_DSN")
    if dsn:
        return psycopg2.connect(dsn)
    password = os.environ.get("VEC_PG_PASSWORD") or get_secret("VEC_PG_PASSWORD")
    if not password:
        raise RuntimeError(
            "Vector Postgres DSN not configured. Set VEC_PG_DSN or VEC_PG_PASSWORD."
        )
    return psycopg2.connect(
        dbname=os.environ.get("VEC_PG_DB", "appdb"),
        user=os.environ.get("VEC_PG_USER", "appuser"),
        password=password,
        host=os.environ.get("VEC_PG_HOST", ""),
        port=int(os.environ.get("VEC_PG_PORT", "5432")),
    )
...
def _openai() -> OpenAI:
    ...
    if not key:
        raise RuntimeError("OPENAI_API_KEY not available (env or Vault vault)")
...
def enqueue_index(
    ...
    q = Queue(queue, connection=Redis(host="", port=6379))
    job = q.enqueue(
